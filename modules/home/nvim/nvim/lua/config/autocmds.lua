...
function MyBufferLine()
  ...
  for idx, buf in ipairs(buffers) do
    -- Only show listed (normal) buffers
    if vim.fn.buflisted(buf) == 1 then
      ...
      if buf_name == '' then
        ...
        -- Extract the file name (relative path can be used if you want)
        buf_name = vim.fn.fnamemodify(buf_name, ':t')
      end

      -- Check if this buffer is the current/active one
      if buf == current_buf then
        -- "%#TabLineSel#" sets the highlight group for the selected buffer
        -- "%X" or "%nT" can make it clickable, but let's keep it simple here
        line = '%#TabLine#' .. line .. '%#TabLineSel# ' .. idx .. ' ' .. buf_name .. ' ' .. '%#TabLine#'
      else
        -- For non-active buffers
        -- The special format `%{N}T` makes the buffer clickable in the tabline
        line = line .. '%' .. buf .. 'T ' .. idx .. ' ' .. buf_name .. ' '
      end
    end
  end
